...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  while (std::getline(ifs, c)) {
    std::vector<std::string> course_vec = split(c, ',');
    Course course_str {course_vec[0], course_vec[1], course_vec[2]};
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  ...
  if (ofs.is_open()) {
    ofs << "Title,Number of Units,Quarter" << '\n';
    for (const auto& elem : unlisted_courses) {
      ofs << elem.title << "," << elem.number_of_units << "," << elem.quarter << '\n';
    }
  }
